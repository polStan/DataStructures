...
class DoublyLinkedList:
    ...
    def insert_last(self, value):
        ...
        if self.head is None: # checking is head is None
            ...
            self.tail = new_node # The tail points to the new node
            return
    ...
    def delete_node(self, value):
        if self.head is not None:
            ...
            return
        while current is not None:# While loop to delete node
            if current.data == value: # If statement to check whether the data is equal to the key that is being checked
    ...
    def search_node(self, value):
        ...
        while current is not None: # While loop to check if current doesn't not have empty data
            if current.data == value: # If the  current data that is in the list equals to the the data that is inputed , it returns True
                return True
            current = current.next
    ...
    def insert_after(self, key, value):
        ...
        while current: # while loop to insert after a given element
            if current.data == key:  # If statement to check if the data is equal to the key that is being searched
                new_node = ListNode(value) # creates a list node containing that data
    ...
    def rotate(self, n):
        if self.head is None or n == 0:
            return
        length = self.length() # get the length of the list
        ...
        while count < n and current: # find the nth node
            current = current.next
            count += 1
    ...
    # Set the new head
        self.head = nth_node.next # head is updated to point to the next node
        self.head.prev = None # Previous is None
    
    # Breaks the circular link
        nth_node.next = None
